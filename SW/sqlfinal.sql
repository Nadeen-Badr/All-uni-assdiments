...
(
    ...
    CONSTRAINT fk_daily_tips_tag FOREIGN KEY (tag) REFERENCES tags(tag),
    CONSTRAINT fk_daily_tips_sub_emotion_id FOREIGN KEY (sub_emotion_id) REFERENCES sub_emotions(sub_emotion_id)
);
...
(
    ...
    CONSTRAINT fk_userplan_activities_user_tag_id FOREIGN KEY (user_tag_id) REFERENCES user_tags(user_tag_id) 
);
...
(
    ...
    gender VARCHAR(50),
    plan_number integer,
    depression_plan_number integer,
...
(
    ...
    tag VARCHAR(255),
    user_id INTEGER,
    CONSTRAINT fk_user_tags_tag FOREIGN KEY (tag) REFERENCES user_preferences_answers(tag),
    CONSTRAINT fk_user_tags_user_id FOREIGN KEY (user_id) REFERENCES users(user_id)
);
CREATE TABLE IF NOT EXISTS userplanfinal
(
    final_plan_id SERIAL PRIMARY KEY,
    plan_number INTEGER,
    activity_plan_id INTEGER,
    done BOOLEAN DEFAULT false,
    CONSTRAINT fk_userplanfinal_plan_number FOREIGN KEY (plan_number) REFERENCES user_Metadata(plan_number),
    CONSTRAINT fk_userplanfinal_activity_plan_id FOREIGN KEY (activity_plan_id) REFERENCES userplan_activities(user_activity_id)
);

CREATE TABLE IF NOT EXISTS depression_activity
(
    activity_id SERIAL PRIMARY KEY,
    description TEXT,
    tag VARCHAR(255),
    CONSTRAINT fk_depression_activity_tag FOREIGN KEY (tag) REFERENCES tags(tag)
);
CREATE TABLE IF NOT EXISTS user_plan_depression
(
    user_plan_depression_id SERIAL PRIMARY KEY,
    user_tag_id INTEGER,
    activity_id INTEGER,
    CONSTRAINT fk_user_plan_depression_user_tag_id FOREIGN KEY (user_tag_id) REFERENCES user_tags(user_tag_id),
    CONSTRAINT fk_user_plan_depression_activity_id FOREIGN KEY (activity_id) REFERENCES depression_activity(activity_id)
);

CREATE TABLE IF NOT EXISTS userdepressionplanfinal
(
    final_depression_plan_id SERIAL PRIMARY KEY,
    depression_plan_number INTEGER,
    depression_activity_plan_id INTEGER,
    done BOOLEAN DEFAULT false,
    CONSTRAINT fk_userplanfinal_plan_number FOREIGN KEY (depression_plan_number) REFERENCES user_Metadata(depression_plan_number),
    CONSTRAINT fk_userplanfinal_depression_activity_plan_id FOREIGN KEY (depression_activity_plan_id) REFERENCES user_plan_depression(user_plan_depression_id)
);
